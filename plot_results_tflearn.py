...
load_train = False
if Train:
    model.fit(X_train, Y_train, n_epoch=10, shuffle=True,show_metric=False, 
              batch_size=64,validation_set=(X_test, Y_test), run_id='wyolo', callbacks=monitorCallback)
elif load_train:
...
for snr in snrs2:
    ...
    Y_predictions=np.zeros((len(SNR_indxs[0]),grid_nums,grid_nums,10+mods))
        
    for xx in range(0,len(SNR_indxs[0])):
        Y_predictions[xx]=model.predict(X_test_snr[xx:xx+1])
